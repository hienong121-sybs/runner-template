...
async function runCli(rawArgs, options = {}) {
  ...
  runCopyTemplateCli(args);
}

function runCopyTemplateCli(rawArgs) {
  ...
  console.log(`mode: ${force ? "force overwrite" : "skip existing files"}`);
  console.log("");
  ...
  for (const relativePath of templateFiles) {
    ...
    if (!fs.existsSync(from)) {
      console.log(`skip: source file not found -> ${relativePath}`);
      skipped += 1;
      continue;
    }
    ...
    if (exists && !force) {
      console.log(`skip: target exists -> ${relativePath}`);
    ...
    try {
      ...
      console.log(`${exists ? "overwrite" : "copy"}: ${relativePath}`);
      console.log(`  from: ${from}`);
      console.log(`  to: ${to}`);
      copied += 1;
    } catch (error) {
...
async function runCreateTunnelCli(rawArgs) {
  ...
  console.log("runner-template-createtunnel");
