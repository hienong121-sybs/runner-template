...
### 4.9 Hotfix default Nginx site still binds :80
- Lỗi ghi nhận tiếp theo:
  - dù `NGINX_PORT=8080`, Nginx vẫn báo `bind() ... :80 failed (98: Address in use)`
- Nguyên nhân:
  - file mặc định của image (`default.conf`) vẫn tồn tại và bị load cùng app config, giữ listen `80`.
- Cách sửa:
  - trước khi gọi `/docker-entrypoint.sh`, xóa các file default site có thể tồn tại:
